...
class MeuGrafo(GrafoMatrizAdjacenciaDirecionado):
    ...
    def __removeVertice(self, V=''):
        if not self.existeVertice(V):
            raise VerticeInvalidoException(f"O vértice {V} não está presente no grafo.")
    ...
    def __encontrarVerticesFonte(self):
        verticesFonte = []
    ...
    def ordenacao_topologica(self):
        ...
        while len(grafoCopia.N) != 0:

            verticesFonte = grafoCopia.__encontrarVerticesFonte()
            ordenacaoTopologica += verticesFonte

            for v in verticesFonte:
                grafoCopia.__removeVertice(v)
